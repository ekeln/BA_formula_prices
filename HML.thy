...
\begin{align*}
    ...
    \psi &::= \neg \varphi \, | \, \varphi.
\end{align*}
Where $I$ denotes an index set. The empty conjunction \textsf{T} $\coloneqq \bigwedge\varnothing$ is usually omitted in writing.\<close>
...
text \<open>Note that in the Isabelle formalization differs from the mathematical definition by including a special formula @{term "TT"} for \textsf{T} as part of the syntax. 
This is to to enable Isabelle to infer that the type @{term "hml"} is not empty. Semantically, @{term "TT"} is synonymous to \<open>\<And>{}\<close>.
...
text \<open>\textit{As discussed, equivalences in LTS can be defined in terms of \textsf{HML} subsets. Two processes are equivalent regarding a subset of HML if they satisfy the same formulas of that subset. 
...
We denote \textnormal{X-equivalence} of two processes $p$ $q$ by $p \sim_X q$. If they processes are equivalent for every formula in HML, they are bisimilar $p \sim_B q$. A formula \textnormal{distinguishes} one state from another if it is true for the former and false for the latter.}\<close>
...
text \<open>$\cdot \sim_X \cdot$ is an equivalence relation.\<close>

lemma subs_equiv_refl: "reflp (HML_subset_equivalent X)"
...
(*>*)
text \<open>If two states are not HML equivalent, there must be a distinguishing formula.\<close>
